                        ...
                        <div class="card">
                            <div class="card-header">
                             ...
                             <div class="card-body card-block">
                                <form action="{{url('/caripendapatan')}}" method="post" enctype="multipart/form-data" class="form-horizontal">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    <div class="row form-group">
                                        <div class="col col-md-3"><label for="date-input" style = "margin-left: 20px; margin-top: 7px" class=" form-control-label">Tanggal :</label></div>
                                        <div class="col-12 col-md-9">
                                            <div class="col-12 col-md-9"><input type="date" id="date-input" name="date-search" class = "form-control"></div>
                                            <!-- <div class="col col-md-3"><label for="date-input" style = "margin-left: -180px; margin-top: 40px" class=" form-control-label">Tanggal Max:</label></div>
                                            <div class="col-12 col-md-9"><input style = "margin-top: -40px"type="date" id="date-search-max" name="date-search-max" class = "form-control"></div> -->
                                        <div class="col-12 col-md-9">
                                            <button type="submit" class="btn btn-primary btn-sm" style="background-color: green; color: white; margin-left: 410px; margin-top: -65px">
                                    ...
                                    @foreach($pemasukkan as $no => $pemasukkans)
                                    <tr scope="row">
                                        <td>{{ $no+1 }}</td>
                                        <td>{{ $pemasukkans->nama }}</td>
                                        <td>{{ $pemasukkans->nominal}}</td>
                                        @if($pemasukkans->kategoris_id == 1)
                                        <td>OVO</td>
                                        @endif
                                        <td>{{ $pemasukkans->keterangan}}</td>
                                        <td>{{ $pemasukkans->created_at}}</td>
                                    </tr>
                                    @endforeach
                                    </table>
                                    </div>
